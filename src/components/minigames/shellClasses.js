...
export function getMinigameShellClasses(compact) {
  ...
  return {
    outer: 'w-full max-w-[1280px] mx-auto bg-indigo-950 p-2 sm:p-4 md:p-6 lg:p-8 font-sans min-h-screen flex items-center justify-center',
    ...
    header: 'w-full h-auto sm:h-24 bg-yellow-400 border-b-[5px] border-black flex flex-col sm:flex-row items-center sm:justify-between px-4 py-3 sm:py-0 gap-3',
    title: 'text-black text-xl sm:text-2xl md:text-3xl font-extrabold uppercase tracking-tight leading-none font-sans',
    subtitle: 'text-black/70 text-[10px] sm:text-xs font-bold tracking-wider font-mono-tech mt-1',
    workArea: 'p-4 sm:p-6 flex flex-col gap-4 w-full min-h-0',
    footer: 'w-full h-auto px-6 py-4 bg-amber-200 border-t-[5px] border-black flex flex-col sm:flex-row justify-between items-center gap-4',
  };
}
